...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Provinsi
      .findOne(_.pick(params, _.keys(Provinsi.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count provinsis.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Provinsi.create(data);

    // Create relational data and return the entry.
    return Provinsi.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Provinsi.associations.map(a => a.alias));
    const data = _.omit(values, Provinsi.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Provinsi.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Provinsi.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Provinsi.attributes).reduce((acc, curr) => {
      switch (Provinsi.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
